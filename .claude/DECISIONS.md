...
## 2026-07-20 — Comments frontend: wire `ModelDiscussionTab` to the live backend

**Context:** `apps/modeling-commons-frontend`. The comment UI (`CommentsSection`/`CommentsPanel`/`useComments`) was built against fixtures with an optimistic-local-only mutation seam; `ModelDiscussionTab` showed a "coming soon" placeholder. Backend routes under `/api/v1/models/:id/comments[/:commentId][/like]` now exist, so the read/write layers were connected end-to-end.

**Decisions:**
1. **Raw `fetch` (not the typed `useApi` client).** The comment routes aren't in `shared/types/api.d.ts` yet, so per the frontend CLAUDE.md convention the read fetchers (`fetchModelComments`/`fetchComment` in `useComments.ts`) and `useCommentMutations.ts` use `fetch(apiBase + path, { credentials: "include" })`. Migrate to `api.METHOD(...)` after `yarn generate:types`.
2. **`CommentsSource` now carries `modelId` always, `commentId` optional.** The backend thread endpoint is `GET /models/:id/comments/:commentId` (needs both). Rule: `commentId` present -> thread mode; else model-list. Dropped the old "exactly one of modelId/commentId" invariant; `CommentsSection` requires a `modelId` and the thread page passes both.
3. **Sort exposes only `Oldest` (`createdAt`) and `Most liked` (`likes`).** The backend hardcodes sort direction (`createdAt`/`updatedAt` ascending, `likes` descending), so "Newest first" (createdAt desc) is impossible without a backend change; `updatedAt` was omitted because ascending updatedAt has no meaningful user-facing label. `filterOptions` -> sort control, "Filter by" -> "Sort by".
4. **Reply paging keeps chronological (default) order, not the top-level sort.** `handleLoad` fetches the next reply page via the re-rooted get-comment endpoint with `limit = maximumShownRepliesPerLevel` and no `sort`, appending deduped replies; passing the top-level `likes` sort would interleave inconsistently with the createdAt-ascending embedded first page.
5. **Mutations are optimistic with snapshot rollback + error toast.** `runOptimistic(apply, persist)` snapshots `local.comments`/`local.pagination`, applies the local change, awaits the API call, and restores the snapshot + toasts on failure. Create/reply swap the temporary `local-*` id for the server id on success. A single `busy` gate serializes mutations (a dropped rapid re-entry is acceptable for v1).
6. **Tests mock `global.fetch` via `installCommentFetchMock` (tests/helpers/comment.ts).** A `toApiComment` reverse-mapper reuses the existing frontend fixtures as backend-shaped responses, so the fixture ids/tree structure still drive the read + mutation assertions.

## 2026-07-24 — Comments: reply paging is server-driven (fixes the dead "See N more replies")

**Context:** Clicking "See N more replies" issued a request and appended nothing. `loadReplies` asked for `lastPage + 1` at the frontend's own page size (`COMMENT_TREE_DEFAULTS.maximumShownRepliesPerLevel = 5`) while the embed came from the backend's (`2`), and `get-comment` ordered the root's reply page by the querystring `sort` (default `likes`) while every embedded reply page is `createdAt asc`. Since the server offsets by `page * limit`, page 1 at limit 5 landed past the end of a 3-reply node — empty response, `lastPage` advanced anyway, button unchanged, forever. Supersedes the 2026-07-20 frontend decision 4 and the 2026-07-10 decision 3 (`COMMENT_TREE_DEFAULTS`).

**Decisions:**
1. **The server owns reply page size; the client echoes it back.** `CommentPagination` gained `limit`, carried from the served page (`toPagination`), and `loadReplies`/`loadMore` send it with the next `page`. The client never names a page size of its own — mismatched limits silently skip rows.
2. **`get-comment` always reads the root's reply page with `EMBED_ORDER_BY`** (`createdAt asc`, now exported from `list-comments.query.ts`). `sort` is gone from `GET /v1/models/:id/comments/:commentId`: it has its own `dtos/get-comment.query.dto.ts` (page/limit only), since paging replies must continue the same list the embed sliced.
