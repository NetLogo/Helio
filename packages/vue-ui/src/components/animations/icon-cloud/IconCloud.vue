<template>
  <canvas
    ...
    width="300"
    height="300"
    :class="cn('rounded-lg w-[300px]', $props.class)"
    role="img"
...
const imageCanvasesRef = ref<Array<HTMLCanvasElement>>([])
const imagesLoadedRef = ref<Array<boolean>>([])

const imagePositions = ref<Array<SphereIcon>>([])
...
watchEffect(() => {
  if (!images) return
  imagesLoadedRef.value = new Array(images.length).fill(false) as Array<boolean>
...
watchEffect(() => {
  ...
  const newPositions: Array<SphereIcon> = []
...
onMounted(() => {
  ...
  function animate(): void {
    ...
    imagePositions.value.forEach((icon, index) => {
      ...
      const scale = ((rotatedZ + 200) / 300) * 2
      ...
      if (imageCanvasesRef.value[index] && imagesLoadedRef.value[index]) {
        ctx.drawImage(imageCanvasesRef.value[index], -20, -20, 80, 80)
      }
      ctx.restore()
    })
