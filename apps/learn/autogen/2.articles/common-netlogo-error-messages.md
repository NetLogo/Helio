...
## Nothing named \[NAME\] has been defined.
...
## OF expected this input to be a reporter block, but got a number instead.

::ErrorBanner
OF expected this input to be a reporter block, but got a number instead.
::

This error occurs when a reporter preceding the `of` primitive is not enclosed in brackets (`[]`). You can refer to the [[of]] dictionary entry to learn more about how to use the `of` primitive correctly.
...
## WITH expected this input to be a TRUE/FALSE block, but got a number instead

::ErrorBanner
WITH expected this input to be a TRUE/FALSE block, but got a number instead.
::

This error occurs when we forget to add brackets around the true/false statement that follows the `with` primitive. You can refer to the [[with]] dictionary entry to learn more about how to use the `of` primitive correctly.
...
## ASK expected this input to be an agent or agentset, but got NOBODY instead.
...
## You can't use ... in a turtle context, because ... is observer only.
...
::
- ::ErrorBanner{.ml-0}
You can't use DIE in a patch context, because DIE is turtle/link only.
::
- ::ErrorBanner{.ml-0}
