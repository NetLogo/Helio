...
See the following page for a list of important changes since NetLogo 6.4:
[NetLogo 7.0.0 Changes Overview](netlogo7intro).

Changes directly affecting models are
[the new file format](netlogo7intro#new-file-format) and
[widget size conversion](netlogo7intro#widget-sizes).
...
`cf:ifelse` and `cf:ifelse-value` primitives are now now supported directly in
NetLogo without an extension by [[ifelse]] and [[ifelse-value]]. Simply remove
the `cf:` from the primitives in your models and they should work as they did
before with CF.

### `ifelse-value` Precedence Change with Infix Operators

In previous versions, `ifelse-value` expressions would be parsed before infix
expressions they were a part of, like `+` or `-`. Now `ifelse-value` has a lower
precedence to let it work with a variable number of clauses.

This used to work, but will now complain about finding a block or list where it
wanted a number, since the `+` is parsed before the `ifelse-value`:

```
ifelse-value (true) [10] [3] + ifelse-value (false) [5] [7]
```

Now you just need to wrap your `ifelse-value` in parenthesis to tell NetLogo
that you want to add the separate expressions:
...
The `arduino` extension has substantially changed the way it receives values
...
version of the `arduino` extension to receive the same message, sketches should
send ";A,D,2.5;" instead. The added 'D,' informs Netlogo that the value being
sent is a number and not a string.

### CF Extension Changes

All primitives have been removed from the `cf` extension and replaced by the
`cf:ifelse` and `cf:ifelse-value` multi-branch primitives. We encourage existing
`cf` users to adjust their code to use these new primitives. Over time, we hope
...
[arrow syntax](dictionary#arrow) for creating anonymous procedures.
...
tasks and it has _not_ been converted, the autoconverter wasn't able to convert
...
- If your code uses `run` or `runresult` to evaluate strings containing tasks,
  ...
  make `run` and `runresult` work as expected, look at strings in your model and
  change any which rely on `task` or `?`-variables to instead rely on anonymous
  procedures
...
would report _all_ links connected to a turtle, whereas `link-neighbors` would
only report neighbors connected by _undirected_ links. Furthermore, it was quite
...
- Link reporters that do not specify "out" or "in" utilize _all_ links.
...
[Vid Extension](vid) section of the manual. As with all extensions, users
...
and easy to develop more _powerful_ extensions. To that end, we've bumped the
...
opening in NetLogo 6. Existing uses of `range` in models authored before NetLogo
6 will be converted to `_range` upon first opening in NetLogo 6. Once the model
opens, you can rename `_range` to suit your model.
...
long lists. See the
[Lists section of the Programming Guide](/programming#lists) for details.
...
Some special ways of writing list-processing code that were useful in NetLogo
4.1 are no longer needed in 5.0. For example, since in 4.1 `fput` was fast and
`lput` was slow, modelers sometimes built up lists in reverse order using
`fput`, perhaps calling `reverse` later to restore the intended order. In
...
The [Extensions Guide](/extensions) has sample code showing the use of
`LogoListBuilder`.
...
[[create-turtles]] ([[crt|create-turtles]]) and [[create-ordered-turtles]]
...
randomize the new turtles' headings, for example `rt random 360` or
`set heading random 360`. These commands are no longer necessary when used
inside `crt`.
...
Links are documented in the [Links](/programming#links) section of the
...
form without underscores ([[create-links-with|create-link]]). However,
...
[[display]] command can be used to force additional updates; see below.)
...
[[tick]] command to your code, otherwise the view won't update. (Note that the
...
See the [Color](/programming#colors) section of the Programming Guide for
details.
...
See the [Tie](/programming#tie) section of the programming guide for
details.
...
"Lists of agents" in the [Lists](/programming#lists) section of the
Programming Guide.
...
the [Topology](/programming#topology) section of the Programming Guide for a
...
[Topology](/programming#topology) section of the Programming Guide for more
information.)
