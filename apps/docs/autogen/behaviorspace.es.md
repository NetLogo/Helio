...
- [**¿Qué es Espacio-Conductual?**](#qu%C3%A9-es-espacio-conductual): Describe la herramienta en términos generales,
  incluyendo las ideas y principios que la sustentan.
- [**Cómo funciona**](#c%C3%B3mo-funciona): Te guía en el uso de la herramienta y destaca
...
- [**Resultados estadísticos**](#resultados-estad%C3%ADsticos): La media y la desviación estándar
...
- [**Opciones de ejecución: actualizar gráficos y monitores**](#opciones-de-ejecuci%C3%B3n-actualizar-gr%C3%A1ficos-y-monitores):
...
- El cuadro de diálogo del experimento ahora usa mensajes emergentes debajo de cada elemento de entrada, en lugar de
  texto; consulta
  [**Creación de una configuración de experimentos**](#creaci%C3%B3n-de-una-configuraci%C3%B3n-de-experimentos).
...
- Modificaciones menores adicionales pueden encontrarse en esta página, buscando _(Desde 6.4)_.
...
[**Importar y exportar**](#importar-y-exportar) (_Desde 6.4_).
...
o ver los deslizadores en la pestaña de "Ejecutar" (_Desde 6.4_).
...
y sobre cómo se utiliza, de modo que no tengas que visitar esta página con tanta frecuencia _(Desde 6.4)_.
...
para garantizar que todos los nombres sean únicos _(Desde 6.4)_.
...
[Random Numbers](/programming#random-numbers) de la Guía de
Programación.
...
como sucedía cuando _Evaluar ejecuciones a cada paso_ (ahora _Ejecutar indicadores a cada paso_)
se dejaba en blanco. Por ejemplo _ticks mod 10 = 0_
registrará datos tanto en cada décimo tick, como en el último tick. Se pueden combinar múltiples
reporteros usando _and_ y _or_.
...
- El encabezado de columna `[initial & final values]` en los resultados en hoja de cálculo se cambió por el más preciso
  `[final value]`.
- Se añadió [**Resultados estadísticos**](#resultados-estad%C3%ADsticos).
...
rendimiento mejora si se deja en blanco. Es importante destacar que _(Desde 6.4)_, si se comienza el experimento
...
el máximo recomendado de ejecuciones en paralelo se indican debajo del cuadro de texto _(Desde 6.4)_. La elección
de la cantidad de ejecuciones en paralelo se mantiene de un experimento a otro _(Desde 6.4)_.
...
entry](/faq#how-big-can-my-model-be-how-many-turtles-patches-procedures-buttons-and-so-on-can-my-model-contain)).
Por defecto, NetLogo no utilizará más del 50% de la memoria del sistema.

Sexta: cada ejecución en paralelo tiene su propio mundo en el que se ejecuta el modelo. Este mundo _no_ se borra automáticamente
...
_(Desde 6.4)_. Para detener el experimento de inmediato, sin esperar a que las ejecuciones actuales se completen,
...
_(Desde 6.4)_, y luego examinar el o los archivos .xml resultantes en un editor de texto.
...
el tamaño del heap](/faq#how-big-can-my-model-be-how-many-turtles-patches-procedures-buttons-and-so-on-can-my-model-contain)
...
a las de Espacio-Conductual. Consulta la sección [Control](#controlling) del Manual del Usuario
