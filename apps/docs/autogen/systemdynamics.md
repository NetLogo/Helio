...
A System Dynamics diagram is made of four kinds of elements: Stocks, Variables,
Flows and Links.
...
There are four basic models in the Sample Models section of the NetLogo Models
...
the Tools menu. The System Dynamics Modeler window will appear. ![[systemdynamics/window.webp]]
...
![[systemdynamics/elementbuttons.webp]]

A System Dynamics diagram is made up of four kinds of components: Stocks,
...
which becomes a global variable. Stocks also require an **Initial value**. It
...
To create a Variable, press the Variable button and click on the diagram. Each
...
**Expression**. This expression can be a number, a variable, a complex NetLogo
...
To create a Flow, press the Flow button or use the right mouse button to Click
...
reporter. Flows require an **Expression**, which is the rate of flow from the
...
the flow is in the opposite direction.

When more than one Flow is connected to a Stock, it is important to consider how
...
**Expression** for a Flow.

For example, if the Flow is defined as a constant value, 10, you can ensure it
never draws more than the value of the Stock by using the [[min]] primitive:
`min (list stock 10)`. If I want Flow A to deplete a Stock before Flow B is
calculated, I can link Flow A to Flow B and modify Flow B to subtract Flow A's
value from the stock: `min (list (max (list 0 (stock - flow-a))) 10)`.
...
To create a Link, press the Link button, or using the right mouse button, click
and hold on the starting point for the link -- a Variable, Stock or Flow -- and
drag the mouse to the destination Variable or Flow.
...
is causing the trouble will be highlighted. ![[systemdynamics/error.webp]]

This should give you a better idea which element in the diagram is causing the
problem. ![[systemdynamics/errordialog.webp]]
...
> - Launch the System Dynamics Modeler in the Tools menu. ![[systemdynamics/tutorial1.webp]]

Our model will have a population of wolves and a population of sheep. Let's
start with the sheep. First, create a Stock that holds a population of Sheep.

> - Press the Stock button in the toolbar. ![[systemdynamics/tutorial2-0.webp]]
> - Click in the diagram area.

You see a Stock with a red question-mark in the middle. ![[systemdynamics/tutorial2-1.webp]]
...
>   negative sheep! ![[systemdynamics/tutorial2-2.webp]]
...
Your diagram should look something like this: ![[systemdynamics/tutorial5.webp]]
...
looks like this: ![[systemdynamics/tutorial8.webp]]
...
>   ![[systemdynamics/tutorial10.webp]]
>
>   where initial-value of `wolves` is `30`, `wolf-deaths` is
>   `wolves * wolf-death-rate`, `wolf-death-rate` is `0.15`,
>   `predator-efficiency` is `.8`, `wolf-births` is
>   `wolves * predator-efficiency * predation-rate * sheep`, `predation-rate` is
>   `3.0E-4`, and `sheep-deaths` is `sheep * predation-rate * wolves`.
>
...
>   appears, enter `0.01`.
>   <p class="screenshot">
>     <img alt="screen shot" style="height: auto; width: auto; max-width: 40%; max-height: 40%;" src="images/systemdynamics/setdtexample.webp" />
...
You see a plot of the populations that looks like this: ![[systemdynamics/tutorial11.webp]]
