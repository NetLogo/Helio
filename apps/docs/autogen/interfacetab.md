...
[Chart: Interface Widgets](#chart-interface-widgets).

The Interface toolbar includes four tool buttons which control the
meaning of user actions. They are the [Interaction Tool](#interaction-tool),
the [Selection Tool](#selection-tool),
the [Edit Tool](#editing-tool), and the [Delete Tool](#deletion-tool).
There are also two pull-down menu tools, [Add Widgets Menu](#add-widgets-menu),
and [Align Widgets Menu](#align-widgets-menu). For details on the use of these
...
One of the four tools is always active unless the user is currently
...
the Delete Tool (a garbage can icon).

Here is a brief introduction to working with widgets.

**Adding:** To add a widget, select it from the "Add Widget" drop-down
...
values. To cancel the addition of the widget, right click or press the Escape key.
...
replaced. The same logic applies to the Output widget.
...
right-clicking it and choosing "Delete" from the popup menu.

**Adjusting:** After selecting multiple widgets you can use the
Align Widget menu to align, stretch or distribute them.
...
Buttons perform actions when clicked. The input widgets set values for
...
<table border>
<colgroup><col style="width: 14%" /><col style="width: 42%" /><col style="width: 42%" /></colgroup>
<thead>
<tr>
...
<p></p>
  ...
  <tr>
    ...
    <td>
      ...
      <a href="dictionary#colorgroup">Color commands or variables</code></a>.
      If the global variable type is String(reporter), it can be evaluated using the
      <a href="dictionary#run"><code>runresult</code></a> primitive.
      If the type is String(commands), it can be executed using the
    <a href="dictionary#run"><code>run</code></a> primitive.
    </td>
  </tr>
  <tr>
...
linear with the amount the slider knob is moved.

- The speed slider lets you control how fast the model runs by
...
- The update mode menu allows you to switch between continuous and tick-based
  ...
  fuller discussion of view updates, see the  [Programming Guide](/programming).)
...
type and place the widget.

The Interaction Tool will be activated once you close the widget's Edit dialog by clicking
“OK” to insert the widget or “Cancel” to abort the process.
...
overlap of widgets. See [Widget Overlap](#widget-overlap) for more details.
...
top and bottom. Widgets can also centered vertically or horizontally.
...
If Stretch Left is selected the widget with the left-most
...
The actions of aligning, distributing or stretching
...
becoming completely hidden behind another.
...
Users can Select, Edit and Delete widgets in interaction mode by
...
Inspect Globals, Inspect a patch, and Inspect, Watch or Follow a turtle.

#### Ctrl-clicking or Cmd-clicking

Selection of a widget can be toggled on and off using Ctrl-click
(Windows and Linux) or Cmd-click (MacOS). Performing this action when
using the Interaction Tool will activate the Selection Tool.

#### Drag Select

One option that is not available with the Interaction Tool is using
...
Pressing the Selection, Editing or Deletion icons
...
These explicit tools are new for NetLogo 7, although they incorporate
behaviors associated with certain NetLogo 6.4 icons.

Each of these tools can be entered by pressing the appropriate icon.
...
the Code Tab will activate the Interaction Tool.

### Selection Tool

This tool allows users to select and subsequently edit or move a
...
Press the “Selection Tool” Icon. Widgets will dim. Clicking on a
...
selection markers appear.

#### Selecting multiple items

You can select multiple interface elements at the same time by including
them in the rectangle you drag. Additional elements can also be added by
using the alternative methods.

#### Deselecting

An individual selected widget can be selected by pressing
...
choosing a different tool will exit Selection tool.

The following actions require that at least one widget is selected:

#### Moving

Select one or more widgets, then drag with your mouse to the new
...
This tool allows users to change the characteristics of an interface
widget Enter this tool by pressing the “Editing Tool” Icon on the
Interface toolbar (a pencil).

*Editing*: Press the “Editing Tool” Icon and click on a widget to edit
it. When the user finishes editing a widget, the Editing Tool remains
active. Clicking another widget will edit it.

If a widget is already selected you can press the editing icon to
...
This tool allows users to delete interface elements. Enter this tool by
...
to delete more items. The view cannot be deleted.
...
Press the widget. You may also delete a widget by right-clicking it
and choosing “Delete” from the popup menu. If you use this latter
method, it is not necessary to select the widget first.
...
and 10 patches to the right of the patch at (0, 0), for a total of 21 patches in each row.
...
[Topology section of the Programming Guide](/programming#topology) for more
information.
...
see the [view updates section](/programming#view-updates) of the Programming
Guide.
...
graphics card. See the [System Requirements](requirements#threed) for
details.
...
([Tutorial #2: Commands](tutorial2) is an introduction to using commands in
the Command Center.)

Let's take a look at the design of the Command Center.
![[interfacetab/command-center.png]]

The smaller box at the bottom is the *Command Center input area*, which is where you can
enter commands. The larger box at the top is the *Command Center output area*, which
displays the results of the typed commands after running them.

To run a command, type it in the input area and press the Return or Enter key. In addition,
the up and down arrow keys can be can used to navigate through the history of your recently
typed commands.
...
to specify which agents run the command you type.
...
To resize the command center, drag the dark gray bar that separates it from the model
interface. Or, click one of the little arrows on the left end of the bar to
make the command center either fill the interface or be hidden altogether.
...
Points added to a plot using `plot` or `plotxy` will not be visible if they are outside
