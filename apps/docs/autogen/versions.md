...
- Loading a model with a slider's lower bound, upper bound or increment set to anything other than a constant would result in a critical error.

## Version 7.0.1 (October 2025)

### Bundled software to open multiple instances of NetLogo on Mac

- A _NetLogo Launcher_ app is now included on macOS to allow opening multiple instances of NetLogo simultaneously (on other platforms, you can already open multiple instances of NetLogo by default)
...
- NetLogo Desktop will now export the latest version of NetLogo Web
  ...
  - Other updates to NetLogo Web are documented [here](https://netlogoweb.org/whats-new), from version v2.12.1 to v2.12.4
...
- Monitors were switching to scientific notation at 10,000,000 instead of at 9,007,199,254,740,993 (2<sup>53</sup>)
...
- A new modernized GUI with both light and dark modes (and a classic mode) ([see below](#changes-to-the-netlogo-gui))
- Greater customizability, with more preferences and settings ([see below](#changes-for-customizability))
- NetLogo models are now stored in a new format with the extension `.nlogox` ([see below](#changes-to-the-netlogo-file-format))
...
- See the [NetLogo 7.0.0 Changes Overview](/netlogo7intro) for more details.
...
- Widget sizes have been adjusted for better aesthetics and readability.
  - All widgets now open with the new widget _styles_, but pre-NetLogo 7 models will open with the old widget _sizes_ by default. However, the new widget styles are not designed to look or function well with the old widget sizes, so model authors are strongly encouraged to update their models to use the new widget sizes.
...
- The `.nlogox` format is extensible and will allow new types of data to be stored in the future.
  ...
  - [Resources](/resource-manager) (e.g., pictures, data files) needed for a model may be stored in the `.nlogox` file in the future.
...
- The NetLogo Graphical User Interface (GUI) has been thoroughly [modernized](#changes-to-the-netlogo-gui).
- The model file format has become [extensible](#changes-to-the-netlogo-file-format).
...
- There are three new pages:
  - [NetLogo 7.0.0 Changes Overview](/netlogo7intro)
  - [Netlogo Preferences](/netlogopreferences)
  - [Color Picker Guide](/colorpicker)
...
- [`nw:generate-preferential-attachment`](http://ccl.northwestern.edu/netlogo/docs/nw#nw:generate-preferential-attachment)
...
- A new
  [section on User Interface primitives](/programming#user-interaction-primitives)
...
- Tasks have been replaced by anonymous procedures. Tasks made use of `?`
  ...
  `[ [?1 ?2] -> ?1 + ?2 ]`. See the [the transition guide](transition) for
  more information.
...
- The various primitives starting with `movie-` have been removed, as has the
  ...
  [The transition guide](transition) provides more details and information.
...
- Several new features have been added to the `nw` extension:
  ...
  - Weighted primitives now take symbolic variable names instead of strings. See
    [the transition guide](transition) for more information.
- The `qtj` extension is no longer bundled with NetLogo. Users are encouraged to
  make use of the [new `vid` extension](vid).
