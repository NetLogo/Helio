...
have obtained or made one.

Note that, in addition to the numerous extensions bundled with NetLogo, there
are many other extensions that can be installed through the
[Extension Manager](extension-manager.html).
...
Using [[extensions]] instructs NetLogo to make the specified extensions'
...
2. The `extensions` folder within your installation of NetLogo. For typical
...
4. The `.bundled` subfolder of the `extensions` folder mentioned in Item 2 (for
   example, `/Applications/NetLogo {{version}}/extensions/.bundled` on Mac OS
   X).

The easiest way to install new extensions is to use the Extension Manager. If an
...
Some extensions depend on additional files. These files will be in the
extension's folder along with the JAR file. The folder may also contain other
files such as documentation and example models.
