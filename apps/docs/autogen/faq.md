...
We'd like to hear from you. (See [Contacting Us](/contact).)
...
"Net" is meant to evoke the decentralized, interconnected nature of the
...
If you use or refer to NetLogo, HubNet or a model from the NetLogo models
library, we ask that you cite it as follows:

_NetLogo itself:_ Wilensky, U. 1999. NetLogo.
http://ccl.northwestern.edu/netlogo/. Center for Connected Learning and
Computer-Based Modeling, Northwestern University. Evanston, IL.

_HubNet:_ Wilensky, U. & Stroup, W., 1999. HubNet.
http://ccl.northwestern.edu/netlogo/hubnet. Center for Connected Learning
...
NetLogo was first created in 1999 by Uri Wilensky at the Center for Connected
...
[GPL (GNU General Public License), version 2](http://www.gnu.org/licenses/gpl-2.0),
...
[netlogo-devel](https://groups.google.com/group/netlogo-devel) group.
...
([CC BY-NC-SA](https://creativecommons.org/licenses/by-nc-sa/3.0/)), which is
...
announce it on the [NetLogo Forum](https://forum.netlogo.org/) and the
...
There is a [BehaviorSpace Guide en Español](/es/behaviorspace).
...
You might also ask the question on the NetLogo
Forum](https://forum.netlogo.org/) and the
[NetLogo Users Group](http://groups.google.com/d/forum/netlogo-users) and/or
search past messages on the group.
...
- **Mac:** Copy the NetLogo directory from the disk image into the Applications
  folder.
- **Windows:** The installer is an MSI which can be run using one of the
  following commands (these may require admin privileges):
...
[NetLogo.cfg](#where-are-the-netlogo-configuration-files) file and modify the
`app.runtime` property to the path of your preferred Java Runtime.

### How come NetLogo won't start up on my Linux machine?

We bundle Bellsoft's OpenJDK-based Java runtime when using NetLogo on Linux. If
...
Some users have reported problems opening NetLogo 6 on Mac OS Sierra and later
...
In a few cases involving a remote server that is behind a proxy, NetLogo users
have observed startup times of several minutes. So far all of these cases have
been on Linux systems.

This happens when NetLogo makes a request over the network to update information
about extensions and their latest versions and there is a problem getting the
response. We offer the following workaround.

Turn off the request to update the extensions information by setting the java
...
If you want to allow the updates again, you can set the value to false, or
...
[Running from the command line](behaviorspace#running-from-the-command-line).
...
processor. See
[Run options: parallel runs](behaviorspace#run-options-parallel-runs).
...
[NetLogo Forum](https://forum.netlogo.org/) and the
[NetLogo Users Group](http://groups.google.com/d/forum/netlogo-users).
...
"._filename_.tmp.nlogo" will be created in the same directory as the NetLogo
model.
...
listening/broadcasting on the _same_ network, so some trial and error may be
...
that network may not support IPv4 multicast, in which case discovery will _not_
work and clients must enter IP addresses manually.

Note that the change made in 6.0.3 simply enable selection of the network on
which clients and servers broadcast/listen on. The HubNet server still listens
on _all_ network connections, so all listed IP addresses given in the server
...
We have tested NetLogo with models that use hundreds of megabytes of RAM and
...
The NetLogo engine has no fixed limits on model size. By default NetLogo sets
Java to use up to half of your available system memory. If your model exceeds
that memory limit, you'll get an `OutOfMemoryError` dialog.
...
increases RAM usage greatly.

If you want to increase the memory limit Java uses, you can do so using the
[NetLogo app configuration files](#where-are-the-netlogo-configuration-files).
...
You can change the percentage number from 50 to whatever you want. Save the file
...
`java-options=-XX:MaxRAMPercentage=50` line with a `#` at the start and add a
new line with `java-options=-Xmx####m`. Replace the `####m` with whatever amount
of memory you like; using `4096m` would limit NetLogo to 4096 megabytes of
...
- **Windows:** The file will typically be in
  `C:\Program Files\NetLogo {{version}}\app`, unless you are running 32 bit
  NetLogo on a 64 bit Windows, then it will be in
  `C:\Program Files (x86)\NetLogo {{version}}\app`.
- **Mac OS X:** The file for NetLogo will be located at:
  `/Applications/NetLogo {{version}}/NetLogo {{version}}.app/Contents/app/NetLogo {{version}}.cfg`
  ...
  choosing "Show Package Contents" from the popup menu.

  Please note that depending on your version of Mac OS X, changing the
  configuration file _may_ break application signing. If this happens, follow
  Apple's directions on [this page](https://support.apple.com/en-us/HT202491) to
  temporarily allow apps from "Anywhere", run NetLogo once, then restore the
  setting to "Mac App Store and Identified Developers".

- **Linux:** The configuration files will be located in the `lib/app` directory
  of your NetLogo installation.

By default, Mac builds of NetLogo bundle a 64-bit JVM. Windows and Linux will
...
that is with the GIS extension. See the [GIS Extension Guide](gis).
...
[NetLogo Users Group](http://groups.google.com/d/forum/netlogo-users)and
participants in the [NetLogo Forum](https://forum.netlogo.org/) may be able to
help with this.

You wil find useful advice in the article
[Improving Execution Speed of Models Implemented in NetLogo](https://www.jasss.org/20/1/3).

The [profiler extension](profiler) is useful for identifying which parts of
...
See the ["Math" section of the Programming Guide](/programming#math) for a
...
1. NetLogo's use of floating point numbers, which can produce small accuracy
   issues. See
   [**Floating point accuracy** in the NetLogo programming guide](/programming#math).
2. Agentsets such as `turtles` are
   [always returned in a random order](/programming#agentsets).
3. Monitors re-run their reporter calculation constantly, even when you are not
   running any model code with a forever button or through the command center.

So the monitor constantly re-runs its `mean [xcor] of turtles` reporter, but the
`turtles` agentset gives the turtles in a random order, and so the
floating-point inaccuracies for `mean` will accumulate in a slightly different
...
You see the same problem doing `sum [xcor] of turtles`,
`variance [xcor] of turtles`, or `standard-deviation [xcor] of turtles` -
...
in the command center repeatedly, without a monitor widget at all.

The fixes are straightforward:

- Sort your numbers before you calculate: `mean sort [xcor] of turtles`,
  `sum sort [xcor] of turtles`, `variance sort [xcor] of turtles`. If the
  ...
  have a really large agentset.
- Change the `Decimal places` setting of your monitors to a number where you
  don't notice the changing values. Since the differences in results should be
  small, this is usually possible.
...
If a list contains only literal values, you can write it down just by putting
square brackets around it, like `[1 2 3]`.
...
[list](dictionary#list) primitive.

## BehaviorSpace

You can find details about using BehaviorSpace at
[BehaviorSpace Guide](behaviorspace) and
[BehaviorSpace Guide en Español](/es/behaviorspace)
...
[Table Output](behaviorspace#table-output) section of the BehaviorSpace
Guide.

### How do I measure runs every n ticks?

Use `mod` in your experiment's
[Run Metrics when](behaviorspace#run-metrics-when) text box, e.g.:

    ticks mod 10 = 0

to measure the run after every 10 ticks.
