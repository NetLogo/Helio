...
http://ccl.northwestern.edu/netlogo/hubnet.html. Center for Connected Learning
...
There is a [BehaviorSpace Guide en Español](behaviorspace-spanish.html).
...
[Running from the command line](behaviorspace.html#running-from-the-command-line).
...
processor. See
[Run options: parallel runs](behaviorspace.html#run-options:-parallel-runs).
...
[Improving Execution Speed of Models Implemented in NetLogo](https://www.jasss.org/20/1/3.html).
...
1. NetLogo's use of floating point numbers, which can produce small accuracy
   issues. See
   [**Floating point accuracy** in the NetLogo programming guide](programming.html#math).
2. Agentsets such as `turtles` are
   [always returned in a random order](programming.html#agentsets).
...
[BehaviorSpace Guide](behaviorspace.html) and
[BehaviorSpace Guide en Español](behaviorspace-spanish.html)
...
[Table Output](behaviorspace.html#table-output) section of the BehaviorSpace
Guide.

### How do I measure runs every n ticks?

Use `mod` in your experiment's
[Run Metrics when](behaviorspace.html#run-metrics-when) text box, e.g.:
