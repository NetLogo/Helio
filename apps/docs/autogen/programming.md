...
order. And every time you use it, the agentset is in a _different_ random order.
...
agentsets that contain only _some_ turtles, _some_ patches or _some_ links. For
...
- Use the [[max-one-of]] or [[min-one-of]] reporters to find out which agent is
...
hold the sets of _all_ turtles and _all_ links. Therefore, these agentsets can
grow.
...
containing just the turtles that _currently_ exist. New turtles don't join when
they're born.
...
Turtle, patch, and link variables are different. Each turtle has its _own_ value
...
is [[a complete list here of built in variables|builtinvariables]].)
...
_out_ of, or _from_, one node and _into_, or _to_, another node. The
relationship of a parent to a child could be modeled as a directed link. An
undirected link appears the same to both nodes, each node has a link _with_
...
- Primitives that have "out" in their name utilize outgoing and undirected
  links. You can think of these as "the links I can use to get _from_ the
  current node _to_ other nodes." In general, these are probably the primitives
  you want to use.
- Primitives that have "in" in their name utilize incoming and undirected links.
  You can think of these as "the links I can use to get _to_ the current node
  _from_ other nodes."
- Primtives that do not specify "in" or "out", or have "with" in their name
  utilize _all_ links, both undirected and directed, incoming and outgoing.
...
breeds using the keywords [[undirected-link-breed]] and [[directed-link-breed]].
Breeded links can be created using the commands
[[create-<breed>-with|create-link]] and [[create-<breeds>-with|create-link]] for
undirected breeds and the commands [[create-<breed>-to|create-link]],
[[create-<breeds>-to|create-link]], [[create-<breed>-from|create-link]], and
[[create-<breeds>-from|create-link]] for directed links.
...
![[programming/circle-layout.webp]]
...
of links to layout the network and not another. ![[programming/radial-layout.webp]]
...
times before the layout is stable.) ![[programming/tutte-layout.webp]]
...
colors. The [[pen-size]] used here was 2: ![[color chart|programming/drawing.webp]]
...
more information, see the Shapes Editor [section](shapes) of this manual.
...
[The NetLogo Dictionary](dictionary) has a complete list of built-in
...
Because agentset members are always read in a random order, when [[ask]] is used
with an agentset each agent will take its turn in a random order. See
[Agentsets](/programming#agentsets) for more information.
...
the [[patch-at]] primitive reporter takes _offsets_: distances, in the x and y
directions, _from_ the first agent. In the example `setup` procedure above, the
...
The semicolon character introduces a comment, which lasts until the end of the
line. There is no multi-line comment syntax.
...
- [[with]], [[with-min]], [[with-max]], [[at-points]], [[in-radius]],
  [[in-cone]], [[who-are-not]]
- [[of]]
...
When an expression contains multiple operators with the same precedence, they
are read in code order left-to-right. Example: `true or false and false` is read
as `(true or false) and false` and so results in `(true) and false`, which is
`false`. This is different than many other programming languages where `or` has
a lower precedence than `and`, so the above statement would be read as
`true or (false and false)`, `true or (false)`, so `true`. You can surround
...
The [[__includes|includes]] keyword allows you to use multiple source files in a single
NetLogo model.
...
[Interface Guide](/codetab#included-files-menu) for more details.
...
A forever button can also be stopped from code. If the forever button directly
...
occur, then advance the tick counter to exactly that time.
...
view use the [[display]] command so the user can see what they are drawing, even
though the tick counter is not advancing.
...
With tick-based updates, the setting is a _ceiling_ on how many updates per
...
can-move? merely returns true if the position _distance_ in front of the turtle
...
[[subject]] reporter. Use the [[reset-perspective]] command to stop the observer
from watching, following, or riding. In the 3D view, the observer also returns
to its default position (above the origin, looking straight down).
...
[[show]], [[type]], and [[write]]. These commands send their output to the
Command Center.
...
- [[write]] lets you print values in a format which can be read back in using
  [[file-read]].
...
The [[print]], [[show]], [[type]], and [[write]] primitives differ on the
following facets:
...
user. These primitives include [user-directory](dictionary#user-directory),
[user-file](dictionary#user-file),
[user-new-file](dictionary#user-new-file),
[user-input](dictionary#user-input),
[user-message](dictionary#user-message),
[user-one-of](dictionary#user-one-of), and
[user-yes-or-no?](dictionary#user-yes-or-no).
...
The [NetLogo Dictionary](dictionary) has a section that lists all of the
...
are resolved by breed in the order they are declared in the Code tab.
...
A few primitives are specific to strings, such as [[is-string?|is-of-type]],
[[substring|subliststring]], and [[word]]:
...
to 2147483647. Once the generator has been "seeded" with the [[random-seed]]
...
after you use [[clear-all]] in your `setup` procedure. Then give that value to
the [[random-seed]] command so it will be used for the rest of the run.
...
You can then add a monitor for the `starting-seed` global or simply output it to
...
Most plots can be created by creating and editing plot widgets in the Interface
tab, as described in its [Plots Section](/interfacetab#plots). However it is
possible to control plots from procedures, although you need to create them in
the Interface tab first.
...
If you want to turn off this feature, edit the plot and uncheck the "Auto
...
of such NetLogo colors. ![[programming/colors.webp]]
...
See the [color](dictionary#color)
...
variables in NetLogo ([[color]] for turtles and links and [[pcolor]] for
...
Example: you can change any turtle from its existing NetLogo color to a sixty
percent transparent version of that color using:

    ask one-of turtles [set color lput 102 extract-rgb color]

Note: because 255 is fully opaque and sixty percent transparent is equal to
forty percent opaque, the correct alpha value is 255 \* .4 = 102. See the
palette extension section below for an alternate implementation.
...
The palette extension offers primitives that give the user more control over
colors. It allows for direct manipulation of RGBA and HSB components without the
requiring list manipulations.

Example: changing the transparency of any turtle to sixty percent transparent
with the palette extension:

    extensions [palette]
    ask one-of turtles [palette:set-transparency 60]

The extension offers similar commands for Red, Green, Blue, Alpha, Hue,
Saturation, and Brightness.

See the [documentation](palette) of the palette extension for more
information.

> **Code Examples:** Palette Example, Color Bubbles, Color Painting, Color
> Reveal

**Color Picker dialog**

The [Color Picker](colorpicker) dialog helps you experiment with and choose
colors. Open it by choosing Color Picker on the Tools Menu.

## Movies

This section describes how to capture an ".mp4" movie of a NetLogo model.

First, use the [`vid:start-recorder`](vid#start-recorder) command to
start the video recorder.

To add a frame to your movie, use either
[`vid:record-view`](vid#record-view) or
[`vid:record-interface`](vid#record-interface), depending on whether
...
- The resolution specified in the call to `vid:start-recorder` _`width height`_
...
[`vid:save-recording`](vid#save-recording). The filename you provide
...
[`vid:record-source`](vid#record-source).

NetLogo movies are exported as H.264-encoded MP4 files. To play an MP4 movie,
you can use [the VLC Player](http://www.videolan.org/vlc/index), a free
