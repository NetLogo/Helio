<template>
  <header class="space-y-4">
    <div class="flex items-start justify-between gap-4">
      <h1 class="text-3xl font-bold text-slate-900 tracking-tight m-0">
        {{ title }}
      </h1>
    ...
    <div class="flex items-center gap-2 text-sm text-slate-500 flex-wrap">
      <div v-if="author" class="flex items-center gap-2">
        ...
        <NuxtLink class="font-medium text-blue-700 hover:underline">
          {{ author.name }}
        </NuxtLink>
      </div>
      <span v-if="author" class="text-slate-300 leading-0 text-2xl">&middot;</span>
      ...
      <template v-if="modelGroup">
        ...
        <span>
          Model Group:
          <NuxtLink class="font-medium text-blue-700 hover:underline">{{ modelGroup }}</NuxtLink>
        </span>
      </template>
    </div>
