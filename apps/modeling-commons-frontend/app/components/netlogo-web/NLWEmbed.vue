<template>
  <UCard
    :ui="{
      body: 'relative rounded-xl overflow-hidden border border-slate-200 bg-slate-50 h-125',
    }"
  >
    <div
      v-if="state === NLWEmbedState.Preview"
      :data-model-url="modelUrl"
      class="w-full h-full flex flex-col my-auto items-center justify-center gap-3 text-slate-400 cursor-pointer"
    >
      <NuxtImg
      ...
      />
      <Icon name="i-lucide-circle-play" class="size-18 text-slate-400 mx-auto" />
      Click to Run Model
    </div>
    <div v-else-if="state === NLWEmbedState.Error" class="space-y-6 text-center">
      <Icon name="i-lucide-x-circle" class="size-12 text-red-400 mx-auto mb-3" />
