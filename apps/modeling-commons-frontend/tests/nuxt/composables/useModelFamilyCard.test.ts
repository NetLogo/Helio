...
describe("useModelFamilyCard", () => {
  it("requests /api/v1/models/{id}/family/card lazily and populates data on execute", async () => {
    apiState.current!.GET.mockResolvedValueOnce(
      apiResult.ok({ id: "family-1", root: null, children: [] }),
    );

    const { data, execute } = await useModelFamilyCard("model-fam");
    await execute();

    expect(apiState.current!.GET).toHaveBeenCalledWith("/api/v1/models/{id}/family/card", {
