...
export interface ModelAuthorRepository {
  ...
  findModelsByUser(userId: string, params: PaginatedQueryParams): Promise<Paginated<ModelAuthorEntity>>;
  insertTx(ctx: TransactionContext, entity: ModelAuthorEntity): Promise<void>;
  updateRoleTx(ctx: TransactionContext, modelId: string, userId: string, role: string): Promise<void>;
